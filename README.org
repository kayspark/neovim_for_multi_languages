...
- *No framework* — previously used LazyVim; replaced with 28 hand-picked plugins + Neovim 0.12 built-ins.
...
- *Fugitive + Gitsigns Git workflow* — stage/commit from Fugitive (=:Git=), hunk ops via Gitsigns, diff review via Diffview.
...
- *Zero plenary.nvim* — removed plenary dependency by replacing Neogit with Fugitive.

** Plugin Count

28 plugins total (including dependencies). See =lua/plugins/= for all specs.
...
Fugitive is the primary Git UI, with Gitsigns for hunk operations and Diffview for diff review.

| Mode | Key          | Action            |
|------+--------------+-------------------|
| n    | =<leader>gg= | Fugitive status   |
| n    | =<leader>gc= | Fugitive commit   |
| n    | =<leader>gb= | Fugitive blame    |
| n    | =<leader>gl= | Fugitive log      |
...
*** Git Workflow (Fugitive + Gitsigns + Diffview)

**** 1) Stage changes
- Stage hunks from code: =gs= (hunk), =gS= (buffer), =gh= (preview first)
- Or open Fugitive status: =<leader>gg= (=:Git=), press =s= to stage, =u= to unstage

**** 2) Commit
- =<leader>gc= or =cc= from Fugitive status buffer
- Write message, =:wq= to confirm

**** 3) Diff review
- =<leader>gd= opens Diffview for full repo diff
- =:DiffviewFileHistory %= for current file history
- =:Gvdiffsplit= for inline diff of current file

**** 4) Push / Pull
- =:Git push=, =:Git pull --rebase=, =:Git fetch= from command line
- Or use =P= / =p= shortcuts from Fugitive status buffer

**** Quick habits
- Before commit: =gh= on important hunks + =<leader>gd= full diff pass
- Before push: =<leader>gg= to confirm clean status
- For blame: =<leader>gb= to see line-by-line history
...
*** JDK environment (JDK 21 only)

Local environment standardized on JDK 21. JDK 8 and 11 have been removed from both Macs.

| Component             | JDK  | Path / Notes                                         |
|-----------------------+------+------------------------------------------------------|
| jenv global           | 21   | =JAVA_HOME= → =~/.jenv/versions/21=                 |
| jdtls runtime         | 22   | Via =~/.config/bin/jdtls-java= wrapper (>=21 required) |
| Gradle daemon         | 21   | =java.import.gradle.java.home= → =JAVA_HOME=        |
| Project compile target | per-project | Set by =sourceCompatibility= in =build.gradle= |

Installed JDKs:
- *JDK 22* — =/Library/Java/JavaVirtualMachines/temurin-22.jdk/= (Adoptium)
- *JDK 21* — =/opt/local/Library/Java/JavaVirtualMachines/jdk-21-eclipse-temurin.jdk/= (MacPorts)
...
2. *Gradle import* uses JDK 21 via =java.import.gradle.java.home= (= =JAVA_HOME=).

3. *Compilation target* is set by =sourceCompatibility= / =targetCompatibility=
   in =build.gradle=. jdtls reads this from Gradle metadata.
   The =configuration.runtimes= list declares JDK 21 as the default runtime.

**** jenv integration
=jenv= global is set to =21=. The =jdtls-java= wrapper checks jenv first;
since jenv now returns JDK 21 (>= 17), it is used directly for the jdtls process.
