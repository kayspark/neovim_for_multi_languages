...
Custom Neovim configuration built on [[https://github.com/LazyVim/LazyVim][LazyVim]] with a focus on multi-language development.
Part of the [[https://github.com/kayspark/.dotfiles][dotfiles]] repo as a git submodule.

| Neovim | 0.11+ (LazyVim framework)                           |
| Theme  | [[https://github.com/kayspark/nvim-nepes][nepes]] (dark/light, transparent)                     |
| Nav    | smart-splits.nvim (cross WezTerm/tmux panes)        |
| Git    | Neogit + Diffview (LazyGit intentionally disabled)   |
| Picker | snacks.picker (Telescope disabled)                   |

** Key Choices

- *Neogit-first Git workflow* — stage/commit/push from Neogit, diff review via Diffview.
- *smart-splits.nvim* — seamless navigation across editor splits and WezTerm/tmux panes.
- *Nepes corporate theme* — matching colorscheme across Emacs, tmux, WezTerm, starship.
- *Language extras* — Rust, TypeScript, YAML are available but commented out in =lua/config/lazy.lua=. Uncomment + =:Lazy sync= to enable.

* Installation

#+begin_src bash
git clone https://github.com/kayspark/neovim_for_multi_languages ~/.config/nvim
nvim  # LazyVim installs plugins on first run
#+end_src

* Key Maps

All custom key maps defined in this repository. Default LazyVim key maps are not listed.
...
Neogit is the primary Git UI. LazyGit integration is intentionally disabled.
...
- [[https://github.com/LazyVim/LazyVim][LazyVim starter kit]]
- [[https://github.com/kayspark/nvim-nepes][Nepes colorscheme]]
- Git signing runbook: =docs/ops/git-signing.md=
- [[https://github.com/joshmedeski/dotfiles.git][Josh Medeski's dotfiles]]
